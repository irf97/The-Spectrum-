...
// Pure classifier inside a 10m venue bubble. Past 10m = OutRange.
...
export function classify(p) {
  ...
  if (p.dist > 10)            { reasons.push('out-of-range'); return result('outrange', 0, reasons); }

  const z = zoneByDistance(p.dist);
  if (!p.stable)            reasons.push('flicker');
  if ((p.signal ?? 1) < 0.5) reasons.push('low-signal');

  const distFactor = 1 - clamp(p.dist / 10, 0, 1);
  ...
  if (!p.stable && (z.key === 'nearby' || z.key === 'room')) {
