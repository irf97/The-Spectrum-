      ...
      <header class="flex items-end justify-between gap-3 flex-wrap">
        <div>
          ...
          <p class="text-sm text-themed-soft mt-1 max-w-2xl">
            Status applies to your <b>${escapeHtml(persona?.name || 'active')}</b> alter ego in ${mode} mode${persona?.roleplay ? ` · as ${escapeHtml(persona.roleplay)}` : ''}.
            Switch alter ego with <span class="kbd">[</span>/<span class="kbd">]</span>, or pick from the top-right pill.
          </p>
        </div>
